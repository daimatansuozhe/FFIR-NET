...
# === 自动获取脚本文件名作为结果子目录名 ===
SCRIPT_NAME = os.path.splitext(os.path.basename(__file__))[0]  # e.g., 'bilstm'
...
class BiLSTM(nn.Module):
    def __init__(self, input_dim, pred_gap, hidden_dim=64, num_layers=2):
        ...
        self.fc = nn.Linear(hidden_dim * 2, pred_gap)

    def forward(self, x):
        out, _ = self.bilstm(x)
        out = out[:, -1, :]  # 取最后一个时间步的输出（前向+后向拼接）
        return self.fc(out)
...
for folder in tqdm(subfolders, desc="子文件夹遍历进度"):
    ...
    for PRED_GAP in tqdm(GAPS, desc=f"{folder} 步长迭代", leave=False):
        ...
        # === 初始化 BiLSTM 模型 ===
        model = BiLSTM(input_dim=len(FEATURE_COLS), pred_gap=PRED_GAP).to(DEVICE)
        ...
        # === 保存模型 ===
        torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"model_{folder}_gap{PRED_GAP}.pth"))

    # === 保存CSV结果 ===
    results_df = pd.DataFrame.from_dict(all_results, orient='index')
    results_df.index = [int(g[1:]) for g in results_df.index]
    results_df = results_df.sort_index()
    results_df.to_csv(os.path.join(SAVE_DIR, f"{folder}_result.csv"))

    print(f"保存 {folder}_result.csv 到 {SAVE_DIR} 完成")
