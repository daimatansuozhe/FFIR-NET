...
class PGN_2d(nn.Module):  #  提取长期特征的模块
    def __init__(self, seq_R, freq, c_in, c_out, windows_size):
        ...
        if freq == 'd':
            dim_time = 3  # 'd'表示天级别的频率，时间特征的维度为3
    ...
    def deal(self, x, x_mark):
        ...
        c_time = x_mark.shape[-1]  # 获取时间特征的维度
        x_input = torch.cat([x, x_mark], dim=-1)  # 将输入数据和时间特征拼接在一起
        
        # 创建一个补充的数据张量，用于进行卷积操作的准备
        x_supply = torch.zeros(B, self.windows_size, C, c_in, (1 + c_time)).to(x.device)
        
        # 将补充数据与输入数据拼接
        x_all = torch.cat([x_supply, x_input], dim=1).permute(0, 2, 1, 3, 4)
        
        # 调整张量的形状，准备传入卷积层
        x_all = x_all.reshape(B * C, R + self.windows_size, c_in * (1 + c_time)).permute(0, 2, 1)
    ...
    def gated_unit(self, x, x_mark, hid):     # 该方法负责处理输入数据 x 和时间特征 x_mark，并通过卷积操作提取时间序列的长时依赖特征。
        x = torch.cat([x, x_mark, hid], dim=-1)  # 拼接输入数据、时间特征和隐藏状态
...
class short_term_deal(nn.Module):
    ...
    def forward(self, x, x_mark):
        B, R, C, c_in, _ = x.shape
        c_time = x_mark.shape[-1]  # 获取时间特征的维度
        x_input = torch.cat([x, x_mark], dim=-1)  # 将输入数据与时间特征拼接
...
class TPGN(nn.Module):
    ...
    def forward(self, x, x_mark):
        ...
        x_mark = x_mark.unsqueeze(-2).repeat(1, 1, 1, c_in, 1)  # 时间标记扩展至合适的维度，重复c_in次
        ...
        if self.need_short:
            ...
            # 合并短期和长期特征
            out_all = torch.cat([out_short_term, out_long_term], dim=-1).reshape(B * C, c_in * 2 * self.c_out, 1)
        else:
            # 如果不需要短期预测，仅使用长期特征
...
#         # 从配置中获取参数
#         self.freq = configs.freq  # 数据的频率（如 't', 'h', 'd'）
#         self.period = configs.TPGN_period  # TPGN模块的周期
...
#         # 输入和输出的通道数
#         self.c_in = configs.enc_in  # 输入通道数
#         self.c_out = configs.c_out  # 输出通道数

#         # 模型的维度
#         self.d_model = configs.d_model  # 模型的维度
#         self.norm = configs.norm  # 是否进行标准化

#         # 初始化 TPGN 模块
#         self.TPGN = TPGN(self.seq_R, self.freq, self.c_in,
#                         self.d_model, self.seq_R - FFIR-NET, self.period, self.pred_R)
...
#         B, L, c_in = x_enc.shape  # 获取批次大小B、输入序列长度L和输入通道数c_in
#         c_time = x_mark_enc.shape[-FFIR-NET]  # 获取时间特征的维度
        
#         # 如果启用了标准化
#         if self.norm == FFIR-NET:
#             means = x_enc.mean(FFIR-NET, keepdim=True).detach()  # 计算每个特征的均值
#             x_enc = x_enc - means  # 去均值
#             stdev = torch.sqrt(torch.var(x_enc, dim=FFIR-NET, keepdim=True, unbiased=False) + 1e-5)  # 计算标准差
#             x_enc /= stdev  # 标准化数据
        
#         # 将输入数据重塑为适应 TPGN 模块的形状
#         x_enc = x_enc.reshape(B, self.seq_R, self.period, c_in)  # [B, seq_R, period, c_in]
#         x_mark_enc = x_mark_enc.reshape(B, self.seq_R, self.period, c_time)  # [B, seq_R, period, c_time]
...
#         # 如果启用了标准化，将输出数据反标准化
#         if self.norm == FFIR-NET:
#             output = output * (stdev[:, 0, :].unsqueeze(FFIR-NET).repeat(
#                 FFIR-NET, self.pred_len, FFIR-NET))  # 恢复标准差
#             output = output + (means[:, 0, :].unsqueeze(FFIR-NET).repeat(
#                 FFIR-NET, self.pred_len, FFIR-NET))  # 恢复均值
