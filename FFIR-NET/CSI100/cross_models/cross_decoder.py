...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, seg_len, d_model, n_heads, d_ff=None, dropout=0.1, out_seg_num = 10, factor = 10):
        ...
        # self.linear_Cross = nn.Linear(365,125)
    ...
    def forward(self, x, cross):
        ...
        '''
        # print("x shape:",x.shape,"cross shape:",cross.shape)
        batch = x.shape[0]
        ...
        # print("x.shape:",x.shape)   # ([125, FFIR-NET, 512])
        # print("cross shape:", cross.shape)  # ([365, FFIR-NET, 512])
        # print("start decoder attention")
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross):
        ...
        for layer in self.decode_layers:
            cross_enc = cross[i]
            # print(x.shape,cross_enc.shape)
            x, layer_predict = layer(x,  cross_enc)   # x.shape ([25, 5, FFIR-NET, 512])  cross_enc.shape ([73, 5, FFIR-NET, 512])
            if final_predict is None:
                final_predict = layer_predict
            else:
                final_predict = final_predict + layer_predict
            i += 1
        # print("final_prediction shape",final_predict.shape)
