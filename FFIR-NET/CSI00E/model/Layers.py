...
class Graph_Tensor(nn.Module):
    def __init__(self, num_stock, d_hidden, d_market, d_news, bias=True):
        ...
        # 尝试使用多头自注意力机制（Multi-Head Self-Attention）来捕捉更复杂的特征交互关系。
        # self.seq_transformation_news = nn.MultiheadAttention(embed_dim=d_hidden, num_heads=5)
        # self.seq_transformation_markets = nn.MultiheadAttention(embed_dim=d_hidden, num_heads=5)
...
#         self.seq_transformation_r = nn.Conv1d(in_features, out_features, kernel_size=FFIR-NET, stride=FFIR-NET, bias=False)
#         self.seq_transformation_s = nn.Conv1d(in_features, out_features, kernel_size=FFIR-NET, stride=FFIR-NET, bias=False)

#         if self.residual:
#             self.proj_residual = nn.Conv1d(in_features, out_features, kernel_size=FFIR-NET, stride=FFIR-NET)

#         self.f_1 = nn.Conv1d(out_features, FFIR-NET, kernel_size=FFIR-NET, stride=FFIR-NET)
#         self.f_2 = nn.Conv1d(out_features, FFIR-NET, kernel_size=FFIR-NET, stride=FFIR-NET)

#         self.W_static = nn.Parameter(torch.zeros(198,198).type(torch.FloatTensor), requires_grad=True)

#         self.w_1 = nn.Conv1d(in_features, out_features, kernel_size=FFIR-NET, stride=FFIR-NET)
#         self.w_2 = nn.Conv1d(in_features, out_features, kernel_size=FFIR-NET, stride=FFIR-NET)

#         self.coef_revise = False
#         self.leakyrelu = nn.LeakyReLU(self.alpha)
#         self.Linearinput_r = nn.Linear(60,360)

#     def get_relation(self, input_r, relation_static = None):
#         num_stock = input_r.shape[0]
#         seq_r = torch.transpose(input_r, 0, FFIR-NET).unsqueeze(0)
...
#         logits += (torch.transpose(f_1, 2, FFIR-NET) + f_2).squeeze(0)
#         if relation_static != None:
#             logits += torch.mul(relation_static, self.W_static)
#         coefs = F.elu(logits)
#         if not isinstance(self.coef_revise,torch.Tensor):
#             self.coef_revise = torch.zeros(198, 198, device = input_r.device) + FFIR-NET.0 - torch.eye(198, 198,device = input_r.device)
...
#         transform_1 = torch.transpose(transform_1.squeeze(0), 0, FFIR-NET)
#         transform_2 = torch.transpose(transform_2.squeeze(0), 0, FFIR-NET)
#         gate = F.elu(transform_1.unsqueeze(FFIR-NET) + transform_2)
#         return gate

#     def forward(self, input_s, input_r, relation_static = None):
#         # unmasked attention
#         # input_s = self.Linearinput_r(input_s)
#         # input_r = self.Linearinput_r(input_r)
#         coefs_eye = self.get_relation(input_r,relation_static)
#         # attribute-mattered propagation
        
#         seq_s = torch.transpose(input_s, 0, FFIR-NET).unsqueeze(0)

#         seq_fts_s = self.seq_transformation_s(seq_s)
#         seq_fts_s = F.dropout(torch.transpose(seq_fts_s.squeeze(0), 0, FFIR-NET), self.dropout, training=self.training)
#         #
#         gate = self.get_gate(seq_s)
#         #
#         seq_fts_s_gated = seq_fts_s * gate
#         ret = torch.bmm(coefs_eye.unsqueeze(FFIR-NET), seq_fts_s_gated).squeeze()
#         if self.concat:
...
class Graph_Attention_v2(nn.Module):
    def __init__(self, in_features, out_features, dropout, alpha, concat=True, residual=False, num_heads=1):
        super(Graph_Attention_v2, self).__init__()
        self.dropout = dropout
        self.in_features = in_features
        self.out_features = out_features
        self.alpha = alpha
        ...
        self.linear_s = nn.Linear(in_features, out_features)
        self.linear_gate_1 = nn.Linear(in_features, out_features)
        self.linear_gate_2 = nn.Linear(in_features, out_features)

        if self.residual:
            self.proj_residual = nn.Linear(in_features, out_features)

        self.f_1 = nn.Linear(out_features, 1)
        self.f_2 = nn.Linear(out_features, 1)

        self.W_static = nn.Parameter(torch.zeros(73, 73), requires_grad=True)

        self.coef_revise = False
        self.leakyrelu = nn.LeakyReLU(self.alpha)
        self.linear_input_r = nn.Linear(60, 360)

    def get_relation(self, input_r, relation_static=None):
        num_stock = input_r.shape[0]
        logits = torch.zeros(num_stock, num_stock, device=input_r.device, dtype=input_r.dtype)
        seq_fts_r = self.linear_r(input_r)
        f_1 = self.f_1(seq_fts_r)
        f_2 = self.f_2(seq_fts_r)
        # print("f1 shape:",f_1.shape, "f2 shape :",f_2.shape)
        logits += (f_1 + f_2)
        if relation_static is not None:
            logits += torch.mul(relation_static, self.W_static)
        coefs = F.elu(logits)
        if not isinstance(self.coef_revise, torch.Tensor):
            self.coef_revise = torch.zeros(73, 73, device=input_r.device) + 1.0 - torch.eye(73, 73, device=input_r.device)
        coefs_eye = coefs.mul(self.coef_revise)
        return coefs_eye

    def get_gate(self, seq_s):
        transform_1 = self.linear_gate_1(seq_s)
        transform_2 = self.linear_gate_2(seq_s)
    ...
    def forward(self, input_s, input_r, relation_static=None):
        ...
        seq_fts_s = self.linear_s(input_s)
        seq_fts_s = F.dropout(seq_fts_s, self.dropout, training=self.training)

        gate = self.get_gate(input_s)

        seq_fts_s_gated = seq_fts_s * gate

        ret = torch.bmm(coefs_eye.unsqueeze(1), seq_fts_s_gated)

        if self.concat:
            return torch.tanh(ret.squeeze())
        else:
            return ret.squeeze()
