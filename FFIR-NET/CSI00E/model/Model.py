...
class VGC_GAN(nn.Module):
    def __init__(self, num_stock, d_market, d_news, d_hidden, hidn_rnn, heads_att, hidn_att, dropout=0, alpha=0.2, t_mix=1):
        ...
        self.conv = nn.Conv1d(in_channels=d_market, out_channels=d_market, kernel_size=2, stride=2)
        self.channel_fc = nn.Linear(d_market, 1)
        self.graph_generator = Graph_Generator(nnodes=num_stock, k=5, dim=hidn_rnn, device='cuda' if torch.cuda.is_available() else 'cpu')
        self.attentions = [Graph_Attention_v2(hidn_rnn, hidn_att, dropout=dropout, alpha=alpha, residual=True, num_heads=heads_att) for _ in range(heads_att)]
        for i, attention in enumerate(self.attentions):
            self.add_module('attention_{}'.format(i), attention)
...
class MagicNet(nn.Module):
    def __init__(self,
        ...
        self.gat = SimpleGATLayer(in_dim=gat_out_per_head * gat_heads, out_dim=gat_out_per_head, heads=gat_heads, dropout=dropout)
        self.hyper = nn.Sequential(
            nn.Linear(gat_out_per_head * gat_heads, gat_out_per_head * gat_heads),
            nn.GELU(),
            nn.Linear(gat_out_per_head * gat_heads, gat_out_per_head * gat_heads)
        )
        self.readout = Readout(in_dim=gat_out_per_head * gat_heads, hidden_dim=readout_hidden, out_dim=num_classes, task='class' if num_classes>1 else 'reg')
        self.dropout = nn.Dropout(dropout)
        self.reset_parameters()
