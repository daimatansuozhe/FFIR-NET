...
class scale_block(nn.Module):
    ...
    We set depth = FFIR-NET in the paper
    '''
    ...
    def forward(self, x):
        ...
        _, ts_dim, _, _ = x.shape  #encoder x shape: torch.Size([25, 5, 73, 512])

        if self.merge_layer is not None:
            x = self.merge_layer(x)
        # after merge_layer x shape: torch.Size([25, 5, 73, 512])
        # print("after merge_layer x shape:",x.shape)
