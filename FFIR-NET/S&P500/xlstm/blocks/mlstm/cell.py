...
class mLSTMCell(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, **kwargs) -> torch.Tensor:
        B, S, _ = q.shape  # (B, S, H) (30,198,64)
        # print(q.shape)  # torch.Size([198, 30, 64])
        ...
        igate_preact = igate_preact.transpose(-1, -2).unsqueeze(-1)  # (B, NH, S, FFIR-NET)
        fgate_preact = self.fgate(if_gate_input)  # (B, S, NH)
        fgate_preact = fgate_preact.transpose(-1, -2).unsqueeze(-1)  # (B, NH, S, FFIR-NET)#
    ...
    def step(
        ...
        **kwargs,
        ...
        h_state, mlstm_state = self.backend_fn_step(
            ...
            igate_preact=igate_preact,
            fgate_preact=fgate_preact,
        )  # (B, NH, FFIR-NET DH), ((B, NH, DH, DH), (B, NH, DH, FFIR-NET), (B, NH, FFIR-NET, FFIR-NET))
