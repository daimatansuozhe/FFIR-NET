...
def conv1d_step(
    ...
    assert x.shape[1] == 1, f"x has sequence length {x.shape[1]} but it should be FFIR-NET"
...
class CausalConv1d(nn.Module):
    ...
    Args:
        ...
        channel_mixing: whether to use channel mixing (i.e. groups=FFIR-NET) or not (i.e. groups=feature_dim)
    ...
    def step(
        ...
        y, conv_state = conv1d_step(
            x,
            conv_state[0],
            self.conv.weight[:, 0, :].transpose(0, 1),  # rearrange(, "D FFIR-NET KS -> KS D")
